  ...
  return new Promise((resolve, reject) => {
    ...
    const checkGIS = () => {
      if (window.google && window.google.accounts && window.google.accounts.oauth2) {
        ...
        pollTimer = setTimeout(checkGIS, 100)
      }
    }

    // タイムアウト時はポーリングも止める（放置すると 100ms 間隔で回り続ける）
...
// セッション復元はモジュール読み込み時の1回だけ。
// useAuth() の中で呼ぶと、この composable を使うコンポーネントの数だけ実行され、
// そのたびにリフレッシュ用タイマーが張り直される
restoreSession()
